...
#ifndef SPARSE_BLOCK_COO_EXT_H
#define SPARSE_BLOCK_COO_EXT_H
...
public:
	...
	BlockYTY_Ext(const MatrixType &Y, const MatrixType &T, const IndexType numZeros, const IndexType actRows)
		: matY(Y), matT(T), nzrs(numZeros), actRows(actRows) {
	}
	...
	IndexType activeRows() const {
		return this->actRows;
	}

	BlockYTY_Ext transpose() const {
		return BlockYTY_Ext(matY, matT.transpose(), nzrs, actRows);
	}
...
public:
	...
	SparseBlockCOO_Ext(const IndexType &rows, const IndexType &cols)
