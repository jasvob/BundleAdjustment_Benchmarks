...
#ifndef EIGEN_DENSE_SUBBLOCK_BLOCKED_SPARSE_QR_H
#define EIGEN_DENSE_SUBBLOCK_BLOCKED_SPARSE_QR_H
...
namespace Eigen {
	...
	template<typename MatrixType, typename OrderingType, int SuggestedBlockCols, bool MultiThreading> class DenseSubblockBlockedSparseQR;
	template<typename DenseSubblockBlockedSparseQRType> struct DenseSubblockBlockedSparseQRMatrixQReturnType;
	template<typename DenseSubblockBlockedSparseQRType> struct DenseSubblockBlockedSparseQRMatrixQTransposeReturnType;
	template<typename DenseSubblockBlockedSparseQRType, typename Derived> struct DenseSubblockBlockedSparseQR_QProduct;
	namespace internal {

		// traits<DenseSubblockBlockedSparseQRMatrixQ[Transpose]>
		template <typename DenseSubblockBlockedSparseQRType> struct traits<DenseSubblockBlockedSparseQRMatrixQReturnType<DenseSubblockBlockedSparseQRType> >
		{
			typedef typename DenseSubblockBlockedSparseQRType::MatrixType ReturnType;
		...
		template <typename DenseSubblockBlockedSparseQRType> struct traits<DenseSubblockBlockedSparseQRMatrixQTransposeReturnType<DenseSubblockBlockedSparseQRType> >
		{
			typedef typename DenseSubblockBlockedSparseQRType::MatrixType ReturnType;
		};

		template <typename DenseSubblockBlockedSparseQRType, typename Derived> struct traits<DenseSubblockBlockedSparseQR_QProduct<DenseSubblockBlockedSparseQRType, Derived> >
		{
			typedef typename Derived::PlainObject ReturnType;
		};

		// DenseSubblockBlockedSparseQR_traits
		template <typename T> struct DenseSubblockBlockedSparseQR_traits {  };
		template <class T, int Rows, int Cols, int Options> struct DenseSubblockBlockedSparseQR_traits<Matrix<T, Rows, Cols, Options>> {
			typedef Matrix<T, Rows, 1, Options> Vector;
		};
		template <class Scalar, int Options, typename Index> struct DenseSubblockBlockedSparseQR_traits<SparseMatrix<Scalar, Options, Index>> {
			typedef SparseVector<Scalar, Options> Vector;
		};
	} // End namespace internal

	  /**
	  * \ingroup DenseSubblockBlockedSparseQR_Module
	  * \class DenseSubblockBlockedSparseQR
	...
	class DenseSubblockBlockedSparseQR : public SparseSolverBase<DenseSubblockBlockedSparseQR<_MatrixType, _OrderingType, _SuggestedBlockCols, _MultiThreading> >
	{
	protected:
		typedef SparseSolverBase<DenseSubblockBlockedSparseQR<_MatrixType, _OrderingType, _SuggestedBlockCols, _MultiThreading> > Base;
	...
	public:
		...
		typedef DenseSubblockBlockedSparseQRMatrixQReturnType<DenseSubblockBlockedSparseQR> MatrixQType;
	...
	public:
		DenseSubblockBlockedSparseQR() : m_analysisIsok(false), m_lastError(""), m_useDefaultThreshold(true), m_useMultiThreading(_MultiThreading), m_hasRowPermutation(false)
		{ }
		...
		* \sa compute()
		*/
		explicit DenseSubblockBlockedSparseQR(const MatrixType& mat) : m_analysisIsok(false), m_lastError(""), m_useDefaultThreshold(true), m_useMultiThreading(_MultiThreading), m_hasRowPermutation(false)
		{
			compute(mat);
		}
		...
		* Q = DenseSubblockBlockedSparseQR<SparseMatrix<double> >(A).matrixQ();
		* \endcode
		...
		*/
		DenseSubblockBlockedSparseQRMatrixQReturnType<DenseSubblockBlockedSparseQR> matrixQ() const
		{
			return DenseSubblockBlockedSparseQRMatrixQReturnType<DenseSubblockBlockedSparseQR>(*this);
		}
		...
		*/
		...
		{
			eigen_assert(m_isInitialized && "The factorization should be called first, use compute()");
			eigen_assert(this->rows() == B.rows() && "DenseSubblockBlockedSparseQR::solve() : invalid number of rows in the right hand side matrix");
		...
		*/
		...
		inline const Solve<DenseSubblockBlockedSparseQR, Rhs> solve(const MatrixBase<Rhs>& B) const
		{
			eigen_assert(m_isInitialized && "The factorization should be called first, use compute()");
			eigen_assert(this->rows() == B.rows() && "DenseSubblockBlockedSparseQR::solve() : invalid number of rows in the right hand side matrix");
			return Solve<DenseSubblockBlockedSparseQR, Rhs>(*this, B.derived());
		}
		template<typename Rhs>
		inline const Solve<DenseSubblockBlockedSparseQR, Rhs> solve(const SparseMatrixBase<Rhs>& B) const
		{
			eigen_assert(m_isInitialized && "The factorization should be called first, use compute()");
			eigen_assert(this->rows() == B.rows() && "DenseSubblockBlockedSparseQR::solve() : invalid number of rows in the right hand side matrix");
			return Solve<DenseSubblockBlockedSparseQR, Rhs>(*this, B.derived());
	...
	protected:
		...
		template <typename, typename > friend struct DenseSubblockBlockedSparseQR_QProduct;
	...
	*/
	...
	void DenseSubblockBlockedSparseQR<MatrixType, OrderingType, SuggestedBlockCols, MultiThreading>::analyzePattern(const MatrixType& mat)
	{
	...
	template <typename MatrixType, typename OrderingType, int SuggestedBlockCols, bool MultiThreading>
	void DenseSubblockBlockedSparseQR<MatrixType, OrderingType, SuggestedBlockCols, MultiThreading>::updateMat(const Index &fromIdx, const Index &toIdx, DenseMatrixType &mat, const Index &blockK) {
	...
	*
	* The function DenseSubblockBlockedSparseQR::analyzePattern(const MatrixType&) must have been called beforehand with
	* a matrix having the same sparsity pattern than \a mat.
	*
	* \param mat The sparse column-major matrix
	*/
	template <typename MatrixType, typename OrderingType, int SuggestedBlockCols, bool MultiThreading>
	void DenseSubblockBlockedSparseQR<MatrixType, OrderingType, SuggestedBlockCols, MultiThreading>::factorize(const MatrixType& mat)
	{
...
//		std::cout << "Loop   " << double(clock() - beginLoop) / CLOCKS_PER_SEC << "s\n";
	...
	* The choice of implementation depends on a template parameter of the DenseSubblockBlockedSparseQR class.
	* The single-threaded implementation cannot work in-place. It is implemented this way for performance related reasons.
	*/
	template <typename DenseSubblockBlockedSparseQRType, typename Derived>
	struct DenseSubblockBlockedSparseQR_QProduct : ReturnByValue<DenseSubblockBlockedSparseQR_QProduct<DenseSubblockBlockedSparseQRType, Derived> >
	{
		typedef typename DenseSubblockBlockedSparseQRType::MatrixType MatrixType;
		typedef typename DenseSubblockBlockedSparseQRType::Scalar Scalar;

		typedef typename internal::DenseSubblockBlockedSparseQR_traits<MatrixType>::Vector SparseVector;

		// Get the references 
		DenseSubblockBlockedSparseQR_QProduct(const DenseSubblockBlockedSparseQRType& qr, const Derived& other, bool transpose) :
		...
		const DenseSubblockBlockedSparseQRType& m_qr;
	...
	*/
	template <typename DenseSubblockBlockedSparseQRType>
	struct DenseSubblockBlockedSparseQR_QProduct<DenseSubblockBlockedSparseQRType, VectorX> : ReturnByValue<DenseSubblockBlockedSparseQR_QProduct<DenseSubblockBlockedSparseQRType, VectorX> >
	{
		typedef typename DenseSubblockBlockedSparseQRType::MatrixType MatrixType;
		typedef typename DenseSubblockBlockedSparseQRType::Scalar Scalar;

		// Get the references 
		DenseSubblockBlockedSparseQR_QProduct(const DenseSubblockBlockedSparseQRType& qr, const VectorX& other, bool transpose) :
			m_qr(qr), m_other(other), m_transpose(transpose) {}
		inline Index rows() const { return m_transpose ? m_qr.rows() : m_qr.cols(); }
		inline Index cols() const { return m_other.cols(); }
	...
	template<typename DenseSubblockBlockedSparseQRType>
	struct DenseSubblockBlockedSparseQRMatrixQReturnType : public EigenBase<DenseSubblockBlockedSparseQRMatrixQReturnType<DenseSubblockBlockedSparseQRType> >
	{
		typedef typename DenseSubblockBlockedSparseQRType::Scalar Scalar;
		...
		explicit DenseSubblockBlockedSparseQRMatrixQReturnType(const DenseSubblockBlockedSparseQRType& qr) : m_qr(qr) {}
		/*DenseSubblockBlockedSparseQR_QProduct<DenseSubblockBlockedSparseQRType, Derived> operator*(const MatrixBase<Derived>& other)
		{
		return DenseSubblockBlockedSparseQR_QProduct<DenseSubblockBlockedSparseQRType,Derived>(m_qr,other.derived(),false);
		}*/
		...
		DenseSubblockBlockedSparseQR_QProduct<DenseSubblockBlockedSparseQRType, SparseMatrix<_Scalar, _Options, _Index>> operator*(const SparseMatrix<_Scalar, _Options, _Index>& other)
		{
			return DenseSubblockBlockedSparseQR_QProduct<DenseSubblockBlockedSparseQRType, SparseMatrix<_Scalar, _Options, _Index>>(m_qr, other, false);
		}
		DenseSubblockBlockedSparseQRMatrixQTransposeReturnType<DenseSubblockBlockedSparseQRType> adjoint() const
		{
			return DenseSubblockBlockedSparseQRMatrixQTransposeReturnType<DenseSubblockBlockedSparseQRType>(m_qr);
		}
		inline Index rows() const { return m_qr.rows(); }
		inline Index cols() const { return m_qr.rows(); }
		// To use for operations with the transpose of Q
		DenseSubblockBlockedSparseQRMatrixQTransposeReturnType<DenseSubblockBlockedSparseQRType> transpose() const
		{
			return DenseSubblockBlockedSparseQRMatrixQTransposeReturnType<DenseSubblockBlockedSparseQRType>(m_qr);
		}
	...
	template<typename DenseSubblockBlockedSparseQRType>
	struct DenseSubblockBlockedSparseQRMatrixQTransposeReturnType
	{
		explicit DenseSubblockBlockedSparseQRMatrixQTransposeReturnType(const DenseSubblockBlockedSparseQRType& qr) : m_qr(qr) {}
		template<typename Derived>
		DenseSubblockBlockedSparseQR_QProduct<DenseSubblockBlockedSparseQRType, Derived> operator*(const MatrixBase<Derived>& other)
		{
			return DenseSubblockBlockedSparseQR_QProduct<DenseSubblockBlockedSparseQRType, Derived>(m_qr, other.derived(), true);
		}
		template<typename _Scalar, int _Options, typename _Index>
		DenseSubblockBlockedSparseQR_QProduct<DenseSubblockBlockedSparseQRType, SparseMatrix<_Scalar, _Options, _Index>> operator*(const SparseMatrix<_Scalar, _Options, _Index>& other)
		{
			return DenseSubblockBlockedSparseQR_QProduct<DenseSubblockBlockedSparseQRType, SparseMatrix<_Scalar, _Options, _Index>>(m_qr, other, true);
		}
		const DenseSubblockBlockedSparseQRType& m_qr;
	};
	...
	namespace internal {
		...
		template<typename DenseSubblockBlockedSparseQRType>
		struct evaluator_traits<DenseSubblockBlockedSparseQRMatrixQReturnType<DenseSubblockBlockedSparseQRType> >
		{
			typedef typename DenseSubblockBlockedSparseQRType::MatrixType MatrixType;
		...
		template< typename DstXprType, typename DenseSubblockBlockedSparseQRType>
		struct Assignment<DstXprType, DenseSubblockBlockedSparseQRMatrixQReturnType<DenseSubblockBlockedSparseQRType>, internal::assign_op<typename DstXprType::Scalar, typename DstXprType::Scalar>, Sparse2Sparse>
		{
			typedef DenseSubblockBlockedSparseQRMatrixQReturnType<DenseSubblockBlockedSparseQRType> SrcXprType;
			...
			static void run(DstXprType &dst, const SrcXprType &src, const internal::assign_op<Scalar, Scalar> &/*func*/)
			{
				...
				//const_cast<DenseSubblockBlockedSparseQRType *>(&src.m_qr)->_sort_matrix_Q();
				dst = DenseSubblockBlockedSparseQR_QProduct<DenseSubblockBlockedSparseQRType, DstXprType>(src.m_qr, idMat, false);
			}
		};

		template< typename DstXprType, typename DenseSubblockBlockedSparseQRType>
		struct Assignment<DstXprType, DenseSubblockBlockedSparseQRMatrixQReturnType<DenseSubblockBlockedSparseQRType>, internal::assign_op<typename DstXprType::Scalar, typename DstXprType::Scalar>, Sparse2Dense>
		{
			typedef DenseSubblockBlockedSparseQRMatrixQReturnType<DenseSubblockBlockedSparseQRType> SrcXprType;
			typedef typename DstXprType::Scalar Scalar;
			typedef typename DstXprType::StorageIndex StorageIndex;
